...
/**
 * HUD summary instructions. Shared by both backends so the local path cannot
 * silently drift from the OpenAI one.
 */
 ...
 * chat-completions server (Ollama, mlx_lm.server, vLLM). Leaving it unset keeps
 * the stock OpenAI Responses API path byte-for-byte, so this is inert unless
 * deliberately configured.
 *
 * @returns {{url: string, model: string, apiKey: string|null, local: boolean}}
 */
export function resolveHudSummaryTarget() {
  const base = String(process.env.GEV_HUD_SUMMARY_BASE_URL || '').trim().replace(/\/+$/, '');
  if (base) {
    return {
      url: `${base}/chat/completions`,
      model: process.env.GEV_HUD_SUMMARY_MODEL || OPENAI_HUD_SUMMARY_MODEL_DEFAULT,
      // Local servers usually need no auth; support one for gateways that do.
      apiKey: process.env.GEV_HUD_SUMMARY_API_KEY || null,
      local: true,
    };
  }
  return {
...
/**
 * Build the upstream request body for a HUD summary.
 *
 * The two backends take different shapes: OpenAI's Responses API uses
 * instructions/input/max_output_tokens, chat-completions uses messages/max_tokens.
 *
 * @param {{model: string, local: boolean}} target
 * @param {unknown} context - Scene context supplied by the client.
 * @returns {object} JSON-serializable request body.
 */
export function buildHudSummaryBody(target, context) {
    ...
    middlewares.use('/api/openai/hud-summary', async (req, res) => {
      ...
      try {
        ...
        const response = await fetch(target.url, {
          method: 'POST',
          headers,
          body: JSON.stringify(buildHudSummaryBody(target, context)),
        });
...
export function extractOpenAiResponseText(data) {
  // Chat-completions shape, used by local OpenAI-compatible servers. Checked
  // first because the Responses API fields below are absent there.
  const chat = data?.choices?.[0]?.message?.content;
  if (typeof chat === 'string' && chat.trim()) {
    return stripReasoningBlock(chat);
  }
 ...
 *
 * Reasoning models served locally can emit one inline instead of in a separate
 * field. Without this, toFiveWordHudSummary would return the first five words
 * of the reasoning rather than the answer.
 *
 * @param {string} value
 * @returns {string}
 */
export function stripReasoningBlock(value) {
  return String(value || '').replace(/^\s*<think>[\s\S]*?<\/think>/i, '').trim();
}
...
/**
 * Realtime session instructions, verbatim.
 *
